...
export class OpenFile extends Fd {
    ...
    fd_seek(offset, whence) {
        ...
        switch (whence) {
            case wasi.WHENCE_SET: calculated_offset = offset; break;
            case wasi.WHENCE_CUR: calculated_offset = this.file_pos + offset; break;
            case wasi.WHENCE_END: calculated_offset = BigInt(this.file.data.byteLength) + offset; break;
            default: return { ret: wasi.ERRNO_INVAL, offset: 0n }
        }
        if (calculated_offset < 0) { return { ret: wasi.ERRNO_INVAL, offset: 0n } }
        this.file_pos = calculated_offset;
        return { ret: 0, offset: this.file_pos }
    }
    fd_tell() { return { ret: 0, offset: this.file_pos } }
    fd_write(data) {
    ...
    constructor(file) { super(); this.file_pos = 0n; this.file = file }
}

export class OpenDirectory extends Fd {
    fd_seek(offset, whence) { return { ret: wasi.ERRNO_BADF, offset: 0n } }
    fd_tell() { return { ret: wasi.ERRNO_BADF, offset: 0n } }
    fd_allocate(offset, len) { return wasi.ERRNO_BADF }
    fd_fdstat_get() { return { ret: 0, fdstat: new wasi.Fdstat(wasi.FILETYPE_DIRECTORY, 0) } }
    fd_readdir_single(cookie) {
        if (cookie == 0n) {
            return { ret: wasi.ERRNO_SUCCESS, dirent: new wasi.Dirent(1n, this.dir.ino, ".", wasi.FILETYPE_DIRECTORY) }
        } else if (cookie == 1n) {
            return { ret: wasi.ERRNO_SUCCESS, dirent: new wasi.Dirent(2n, this.dir.parent_ino(), "..", wasi.FILETYPE_DIRECTORY) }
        }
        ...
        return { ret: 0, dirent: new wasi.Dirent(cookie + 1n, entry.ino, name, entry.stat().filetype) }
    }
    path_filestat_get(flags, path_str) {
        const { ret: path_err, path } = Path.from(path_str);
        if (path == null) { return { ret: path_err, filestat: null } }
        const { ret, entry } = this.dir.get_entry_for_path(path);
        if (entry == null) { return { ret, filestat: null } }
        return { ret: 0, filestat: entry.stat() }
    }
    path_lookup(path_str, dirflags) {
        const { ret: path_ret, path } = Path.from(path_str);
        if (path == null) { return { ret: path_ret, inode_obj: null } }
        const { ret, entry } = this.dir.get_entry_for_path(path);
        if (entry == null) { return { ret, inode_obj: null } }
    ...
    path_open(dirflags, path_str, oflags, fs_rights_base, fs_rights_inheriting, fd_flags) {
        const { ret: path_ret, path } = Path.from(path_str);
        if (path == null) { return { ret: path_ret, fd_obj: null } }
        let { ret, entry } = this.dir.get_entry_for_path(path);
        if (entry == null) {
            if (ret != wasi.ERRNO_NOENT) { return { ret, fd_obj: null } }
            if ((oflags & wasi.OFLAGS_CREAT) == wasi.OFLAGS_CREAT) {
                ...
                if (new_entry == null) { return { ret, fd_obj: null } }
                entry = new_entry
            } else { return { ret: wasi.ERRNO_NOENT, fd_obj: null } }
    ...
    path_create_directory(path) { return this.path_open(0, path, wasi.OFLAGS_CREAT | wasi.OFLAGS_DIRECTORY, 0n, 0n, 0).ret }
    path_link(path_str, inode, allow_dir) {
        const { ret: path_ret, path } = Path.from(path_str);
        if (path == null) { return path_ret }
        if (path.is_dir) { return wasi.ERRNO_NOENT }
        ...
        if (entry != null) {
            ...
            if (source_is_dir && target_is_dir) {
                if (allow_dir && entry instanceof Directory) {
                    if (entry.contents.size == 0) { } else { return wasi.ERRNO_NOTEMPTY }
                } else { return wasi.ERRNO_EXIST }
            } else if (source_is_dir && !target_is_dir) {
                return wasi.ERRNO_NOTDIR
            } else if (!source_is_dir && target_is_dir) {
                return wasi.ERRNO_ISDIR
            } else if (inode.stat().filetype == wasi.FILETYPE_REGULAR_FILE && entry.stat().filetype == wasi.FILETYPE_REGULAR_FILE) {
            } else { return wasi.ERRNO_EXIST }
        }
        if (!allow_dir && inode.stat().filetype == wasi.FILETYPE_DIRECTORY) { return wasi.ERRNO_PERM }
        parent_entry.contents.set(filename, inode);
        if (inode instanceof Directory) inode.parent = parent_entry;
        parent_entry.notifyMutation();
        return wasi.ERRNO_SUCCESS
    }
    path_unlink(path_str) {
        ...
        if (path == null) { return { ret: path_ret, inode_obj: null } }
        const { ret: parent_ret, parent_entry, filename, entry } = this.dir.get_parent_dir_and_entry_for_path(path, true);
        if (parent_entry == null || filename == null) { return { ret: parent_ret, inode_obj: null } }
        if (entry == null) { return { ret: wasi.ERRNO_NOENT, inode_obj: null } }
    ...
    path_unlink_file(path_str) {
        ...
        if (entry.stat().filetype === wasi.FILETYPE_DIRECTORY) { return wasi.ERRNO_ISDIR }
        parent_entry.contents.delete(filename);
        parent_entry.notifyMutation();
        return wasi.ERRNO_SUCCESS
    }
    path_remove_directory(path_str) {
        ...
        const { ret: parent_ret, parent_entry, filename, entry } = this.dir.get_parent_dir_and_entry_for_path(path, false);
        if (parent_entry == null || filename == null || entry == null) { return parent_ret }
        if (!(entry instanceof Directory) || entry.stat().filetype !== wasi.FILETYPE_DIRECTORY) { return wasi.ERRNO_NOTDIR }
        if (entry.contents.size !== 0) { return wasi.ERRNO_NOTEMPTY }
        if (!parent_entry.contents.delete(filename)) { return wasi.ERRNO_NOENT }
        parent_entry.notifyMutation();
        return wasi.ERRNO_SUCCESS
    }
    fd_filestat_get() { return { ret: 0, filestat: this.dir.stat() } }
    fd_filestat_set_size(size) { return wasi.ERRNO_BADF }
    fd_read(size) { return { ret: wasi.ERRNO_BADF, data: new Uint8Array } }
    fd_pread(size, offset) { return { ret: wasi.ERRNO_BADF, data: new Uint8Array } }
    fd_write(data) { return { ret: wasi.ERRNO_BADF, nwritten: 0 } }
    fd_pwrite(data, offset) { return { ret: wasi.ERRNO_BADF, nwritten: 0 } }
    constructor(dir) { super(); this.dir = dir }
}

export class PreopenDirectory extends OpenDirectory {
    fd_prestat_get() { return { ret: 0, prestat: wasi.Prestat.dir(this.prestat_name) } }
    constructor(name, contents) {
        super(new Directory(contents));
        this.prestat_name = name;
        this.dir.is_root = true;
    }
}
...
export class File extends Inode {
    ...
    get size() { return BigInt(this.data.byteLength) }
    stat() {
        return new wasi.Filestat(this.ino, wasi.FILETYPE_REGULAR_FILE, this.size)
    }
    notifyMutation() {
        if (this.parent) this.parent.notifyMutation();
    }
...
export class Path {
        ...
        for (const part of rawParts) {
            ...
            if (part === "..") {
                if (self.parts.length > 0) {
                    if (self.parts[self.parts.length - 1] === "..") {
                        self.parts.push("..");
                    } else {
                        self.parts.pop();
                    }
    ...
    to_path_string() {
        ...
        if (this.is_dir && this.parts.length > 0) s += "/";
        return s;
    }
    constructor() {
        this.parts = [];
...
export class Directory extends Inode {
    ...
    stat() { return new wasi.Filestat(this.ino, wasi.FILETYPE_DIRECTORY, 0n) }
    ...
    get_parent_dir_and_entry_for_path(path, allow_undefined) {
        ...
        if (filename === undefined) {
            return { ret: wasi.ERRNO_INVAL, parent_entry: null, filename: null, entry: null }
        }
        ...
        if (!(parent_entry instanceof Directory)) {
            return { ret: wasi.ERRNO_NOTDIR, parent_entry: null, filename: null, entry: null }
        }
        const entry = parent_entry.contents.get(filename);
        if (entry === undefined) {
            if (!allow_undefined) {
                return { ret: wasi.ERRNO_NOENT, parent_entry: null, filename: null, entry: null }
            } else {
    ...
    create_entry_for_path(path_str, is_dir) {
        const { ret: path_ret, path } = Path.from(path_str);
        if (path == null) { return { ret: path_ret, entry: null } }
