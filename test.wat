(module
  ;; Imports from the host bridge
  (import "chrome:bookmarks/bookmarks" "get-tree" (func $get_tree (result i32)))
  (import "env" "log" (func $log (param i32 i32)))

  ;; Initial memory of 4 pages (256KB), can grow
  (memory (export "memory") 4)

  ;; cabi_realloc is required by the host for allocations
  ...
  (func $find_leaf (param $node_ptr i32) (result i32)
    ...
    ;; 1. Check if node has a URL (offset 28)
    (local.set $url_tag (i32.load (i32.add (local.get $node_ptr) (i32.const 28))))
    (if (local.get $url_tag)
      (then
        ;; Found leaf! Log the URL (ptr at 32, len at 36)
        (call $log 
          (i32.load (i32.add (local.get $node_ptr) (i32.const 32)))
          (i32.load (i32.add (local.get $node_ptr) (i32.const 36)))
        )
        ;; Return the length of the found URL
        (return (i32.load (i32.add (local.get $node_ptr) (i32.const 36))))
      )
    )

    ;; 2. Check if node has children (offset 40)
    (local.set $child_tag (i32.load (i32.add (local.get $node_ptr) (i32.const 40))))
    (if (local.get $child_tag)
      (then
  ...
  (func (export "main") (result i32)
    ...
    (if (i32.eqz (i32.load (local.get $res_ptr))) ;; Success tag 0
      (then
        ...
        (loop $root_iter
          (if (i32.lt_u (local.get $i) (local.get $list_len))
            (then
              (call $find_leaf (i32.load (i32.add (local.get $list_ptr) (i32.mul (local.get $i) (i32.const 4)))))
              (local.set $found)
              (if (i32.gt_s (local.get $found) (i32.const 0))
                (then (return (local.get $found))))
